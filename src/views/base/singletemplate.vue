<template>
  <el-row id="base_singletemplate">
    ...
    <cfda-template v-model="current.content" :edit="edit"></cfda-template>
...
export default {
  ...
  methods: {
    async init() {
      ...
      this.title = this.isNew ? "新建检查模板" : this.origin.name;
    },
    ...
    async editOK() {
      ...
      if (this.isNew) {
        this.$router.push(`/base/template/${this.origin._id}`);
      } else {
        this.init();
      }
    },
