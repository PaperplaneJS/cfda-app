<template>
  <div id="daily_singlemonitor">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item :to="`/daily/report/${currentReport.plan.id}`">{{currentReport.plan.name}}</el-breadcrumb-item>
    ...
    <el-form label-position="left" style="margin-top:20px;" label-width="100px">
      ...
      <el-row style="font-size:18px;margin-bottom:15px;" class="section">检查内容结果</el-row>
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="检查表:">
            <el-input v-model="currentReport.checkContent" resize="none" readonly></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row :gutter="20">
        <el-col :span="20">
          <div>
            <el-tag type="success">
              符合{{getResult(currentReport.checkdetail)["符合"].total}}项
            </el-tag>
          </div>
          <div style="margin-top:10px;">
            <el-tag style="margin-right:10px" type="warning">
              基本符合{{getResult(currentReport.checkdetail)["基本符合"].total}}项(含{{getResult(currentReport.checkdetail)["基本符合"].important}}个重点项)
            </el-tag>
            <el-popover v-for="item of resultArray(currentReport.checkdetail)['基本符合']" :key="item.index" placement="top-start" :title="`基本符合 +${item.point}`" width="400" trigger="hover" :content="item.detail">
              <el-button style="margin:0 5px;" size="mini" slot="reference">{{item.index}}</el-button>
            </el-popover>
          </div>
          <div style="margin-top:10px;">
            <el-tag style="margin-right:10px" type="danger">
              不符合{{getResult(currentReport.checkdetail)["不符合"].total}}项(含{{getResult(currentReport.checkdetail)["不符合"].important}}个重点项)
            </el-tag>
            <el-popover v-for="item of resultArray(currentReport.checkdetail)['不符合']" :key="item.index" placement="top-start" :title="`不符合 +${item.point}`" width="400" trigger="hover" :content="item.detail">
              <el-button style="margin:0 5px;" size="mini" slot="reference">{{item.index}}</el-button>
            </el-popover>
          </div>
        </el-col>
